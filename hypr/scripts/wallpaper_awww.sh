...
WALLPAPER_ENGINE="${WALLPAPER_ENGINE:-awww}"
...
if [ ! -f $cache_file ] ;then
    touch $cache_file
    echo "$HOME/Pictures/wallpapers/anime-girl-countryside-scenery.png" > "$cache_file"
fi

# Create rasi file if not exists
if [ ! -f $rasi_file ] ;then
    touch $rasi_file
    echo "* { current-image: url(\"$HOME/Pictures/wallpapers/anime-girl-countryside-scenery.png\", height); }" > "$rasi_file"
fi
...
case $1 in
    ...
    "select")
        sleep 0.2
        selected=$(find "$HOME/Pictures/wallpapers/" -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" \) \
        ...
        wallpaper="$HOME/Pictures/wallpapers/$selected"
    ;;
...
matugen image "$wallpaper" --type "scheme-$scheme_type" --source-color-index 0
